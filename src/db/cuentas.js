        ...
        request.onsuccess = async (event) => {
            ...
            if (cursor) {
                ...
                for (const a of resultados) {
                    a.movements = movements.data.filter(b => b.cuenta == a.id)
                }
...
async function delet(data) {
    ...
    if (trymovimientos.data.some(a => a.cuenta == data.id)) {
        swal('error', 'La cuenta tiene registros')
        return {code:1, msg:'La cuenta tiene registros'}
    }
